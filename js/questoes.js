export default [
  {
    questao: "Qual é o país com a maior área territorial do mundo?",
    alternativas: [
      { opcao: "China", correto: false },
      { opcao: "Rússia", correto: true },
      { opcao: "EUA", correto: false },
      { opcao: "Brasil", correto: false }
    ]
  },

  {
    questao: "Quem foi o cientista que formulou a teoria da relatividade?",
    alternativas: [
      { opcao:"Benjamin Franklin", correto: false},
      { opcao:"Thomas Edison", correto: false},
      { opcao:"Isaac Newton", correto: false},
      { opcao:"Albert Einstein", correto: true}
    ]
  },

  {
    questao: "Qual é o nome do famoso quadro de Leonardo da Vinci que retrata a Última Ceia de Jesus com seus discípulos?",
    alternativas: [
      { opcao:"A Virgem das Rochas", correto: false},
      { opcao:"Monalisa", correto: false},
      { opcao:"A Última Ceia", correto: true},
      { opcao:"Homem Vitruviano", correto: false}
    ]
  },

  {
    questao: "Qual é o maior órgão do corpo humano?",
    alternativas: [
      { opcao:"Pele", correto: true},
      { opcao:"Cérebro", correto: false},
      { opcao:"Coração", correto: false},
      { opcao:"Estômago", correto: false}
    ]
  },

  {
    questao: `Quem escreveu a obra "Dom Quixote"?`,
    alternativas: [
      { opcao:"Mark Twain", correto: false},
      { opcao:"Virginia Woolf", correto: false},
      { opcao:"Machado de Assis", correto: false},
      { opcao:"Miguel de Cervantes", correto: true}
    ]
  },

  {
    questao: `Em que continente está localizada a maior parte da Amazônia?`,
    alternativas: [
      { opcao:"América do Sul", correto: true},
      { opcao:"América do Norte", correto: false},
      { opcao:"América Central", correto: false},
      { opcao:"Caribe", correto: false}
    ]
  },

  {
    questao: `Qual é o nome da maior cadeia montanhosa do mundo?`,
    alternativas: [
      { opcao:"Codilheira Australiana", correto: false},
      { opcao:"Codilheira dos Andes", correto: false},
      { opcao:"Codilheira do Himalaia", correto: true},
      { opcao:"Montanhas Rochosas", correto: false}
    ]
  },

  {
    questao: `Quem foi o líder sul-africano que lutou contra o apartheid e se tornou o primeiro presidente negro do país?`,
    alternativas: [
      { opcao:"Martin Luther King Jr.", correto: false},
      { opcao:"Barack Obama", correto: false},
      { opcao:"Nelson Mandela", correto: true},
      { opcao:"Jesse Jackson", correto: false}
    ]
  },

  {
    questao: `Qual é o maior animal terrestre do mundo?`,
    alternativas: [
      { opcao:"Gorila.", correto: false},
      { opcao:"Elefante Africano", correto: true},
      { opcao:"Girafa", correto: false},
      { opcao:"Camelo", correto: false}
    ]
  },

  {
    questao: `Quais são os quatro elementos da natureza na filosofia antiga?`,
    alternativas: [
      { opcao:"Terra, Água, Raio e Fogo.", correto: false},
      { opcao:"Poeira, Água, Ar e Fogo", correto: false},
      { opcao:"Terra, Água, Ar e Fogo", correto: true},
      { opcao:"Terra, Madeira, Ar e Fogo", correto: false}
    ]
  },
];
/*
export default [
    {
      question: "Quanto é 1 + 2?",
      answers: [
        { option: "3", correct: true },
        { option: "2", correct: false },
        { option: "4", correct: false },
      ],
    },
    {
      question: "Quanto é 5 x 2?",
      answers: [
        { option: "9", correct: false },
        { option: "10", correct: true },
        { option: "11", correct: false },
      ],
    },
    {
      question: "Quanto é 10 x 8?",
      answers: [
        { option: "800", correct: false },
        { option: "90", correct: false },
        { option: "80", correct: true },
      ],
    },
    {
      question: "Quanto é 7 x 7?",
      answers: [
        { option: "70", correct: false },
        { option: "56", correct: false },
        { option: "49", correct: true },
      ],
    },
  ];
  */